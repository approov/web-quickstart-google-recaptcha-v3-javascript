# Approov Quickstart: Web - Javascript - reCAPTCHA
...
Add these functions which initialize the Approov SDK, get a reCAPTCHA token and request an Approov token as required:
...
Customize the function using your Approov site key and reCAPTCHA site key to replace `your-Approov-site-key` and `your-reCAPTCHA-site-key`, respectively.
...
A reCAPTCHA site can be added by specifying a valid site key and the associated API secret. The following command adds a site leaving all other settings at their default values:
...
When adding Approov with reCAPTCHA into your own web app you may want to address some of the following points:
